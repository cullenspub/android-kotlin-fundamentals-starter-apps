...
fun sleepText(item: SleepNight): String {
    val sb = StringBuffer()
    sb.apply {
        //
        append("Length: HH:MM:SS - ")
        // Hours
        append("${item.endTimeMilli.minus(item.startTimeMilli) / 1000 / 60 / 60}:")
        // Minutes
        append("${item.endTimeMilli.minus(item.startTimeMilli) / 1000 / 60}:")
        // Seconds
        append("${item.endTimeMilli.minus(item.startTimeMilli) / 1000}")
    }
    return sb.toString()
}
